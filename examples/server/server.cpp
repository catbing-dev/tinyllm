...
struct server_slot {
    ...
    void print_timings() const {
        ...
        sprintf(buffer, "          total time = %10.2f ms",
...
struct llama_server_context {
    ...
    void validate_model_chat_template(server_params &sparams) {
        ...
        int res = llama_chat_apply_template(model, nullptr, chat, 1, true,
                                            buf.data(), buf.size());
        if (res < 0) {
            LOG_ERROR("The chat template comes with this model is not yet "
                      "supported, falling back to chatml. This may cause the "
                      "model to output suboptimal responses",
                      {});
            sparams.chat_template =
                "<|im_start|>"; // llama_chat_apply_template only checks if
                                // <|im_start|> exist in the template
        }
    }
    ...
    void initialize() {
        ...
        for (int i = 0; i < params.n_parallel; i++) {
            ...
            LOG_INFO("new slot",
                     {{"slot_id", slot.id}, {"n_ctx_slot", slot.n_ctx}});
            ...
            if (ga_n != 1) {
                ...
                LOG_INFO(
                    "slot self-extend",
                    {{"slot_id", slot.id}, {"ga_n", ga_n}, {"ga_w", ga_w}});
            }
    ...
    std::vector<llama_token> tokenize(const json &json_prompt,
        ...
        if (json_prompt.is_array()) {
            ...
            prompt_tokens =
                ::llama_tokenize(ctx, s, add_bos, TMP_FORCE_SPECIAL);
        }
    ...
    bool launch_slot_with_data(server_slot *&slot, json data) {
        ...
        if (penalty_prompt != data.end()) {
            if (penalty_prompt->is_string()) {
                const auto penalty_prompt_string =
                    penalty_prompt->get<std::string>();
                auto penalty_tokens =
            ...
            for (const auto &el : *logit_bias) {
                if (el.is_array() && el.size() == 2) {
                    ...
                    if (el[0].is_number_integer()) {
                        ...
                        auto toks = llama_tokenize(
                            model, el[0].get<std::string>(), false);
                        for (auto tok : toks) {
                            slot->sparams.logit_bias[tok] = bias;
                        }
                    }
        ...
        if (!system_prompt.empty()) {
            ...
            for (int32_t i = 0; i < (int32_t)batch.n_tokens;
                ...
                const int32_t n_tokens =
                    std::min(params.n_batch, (int32_t)(batch.n_tokens - i));
    ...
    bool process_token(completion_token_output &result, server_slot &slot) {
        ...
        if (slot.ctx_sampling->params.use_penalty_prompt_tokens &&
            ...
            slot.ctx_sampling->params.penalty_prompt_tokens.push_back(
                result.tok);
        }
        ...
        for (unsigned i = 1; i < 5 && i <= slot.generated_text.size(); ++i) {
            unsigned char c =
                slot.generated_text[slot.generated_text.size() - i];
            if ((c & 0xC0) == 0x80) {
    ...
    json get_formated_generation(server_slot &slot) {
        ...
        for (const auto &sampler_type : slot.sparams.samplers_sequence) {
            samplers_sequence.emplace_back(
                sampler_type_to_name_string(sampler_type));
        }
        ...
        return json{
            ...
            {"use_penalty_prompt_tokens",
             slot.sparams.use_penalty_prompt_tokens},
    ...
    void send_partial_response(server_slot &slot, completion_token_output tkn) {
        ...
        if (slot.sparams.n_probs > 0) {
            ...
            size_t probs_pos = std::min(slot.n_sent_token_probs,
    ...
    void request_completion(int task_id, json data, bool infill, bool embedding,
        ...
        if (task.data.count("prompt") != 0 &&
            ...
            if (task.data.contains("prompt") &&
                task.data["prompt"].is_string() &&
                task.data["prompt"].get<std::string>().empty()) {
                task.data["prompt"] =
                    " "; // add a space so that we have one token
            }
            queue_tasks.post(task);
        }
    }
    ...
    void split_multiprompt_task(int multitask_id,
                                task_server &multiprompt_task) {
        int prompt_count = multiprompt_task.data.at("prompt").size();
        if (prompt_count <= 1) {
            send_error(multiprompt_task,
                       "error while handling multiple prompts");
            return;
        }
    ...
    void process_single_task(task_server &task) {
        ...
        case TASK_TYPE_COMPLETION: {
            ...
            if (slot == nullptr) {
                // if no slot is available, we defer this task for processing
                // later
            ...
            if (task.data.contains("system_prompt")) {
                if (!all_slots_are_idle) {
                    send_error(task, "system prompt can only be updated when "
                                     "all slots are idle");
                    break;
                }
        ...
        case TASK_TYPE_METRICS: {
            ...
            LOG_INFO("slot data", {{"task_id", task.id},
    ...
    bool update_slots() {
        ...
        if (all_slots_are_idle) {
            if (system_prompt.empty() && clean_kv_cache) {
                LOG_INFO("all slots are idle and system prompt is empty, clear "
                         "the KV cache",
                         {});
                kv_cache_clear();
            }
        ...
        for (server_slot &slot : slots) {
            if (slot.ga_n == 1) {
                if (slot.is_processing() &&
                    ...
                    const int n_left =
                        (int)system_tokens.size() + slot.n_past - n_keep;
                    ...
                    llama_kv_cache_seq_rm(ctx, slot.id, n_keep,
                                          n_keep + n_discard);
                    llama_kv_cache_seq_add(ctx, slot.id, n_keep + n_discard,
                                           system_tokens.size() + slot.n_past,
                                           -n_discard);

                    for (size_t i = n_keep + n_discard;
                         i < slot.cache_tokens.size(); i++) {
                        slot.cache_tokens[i - n_discard] = slot.cache_tokens[i];
                    }

                    slot.cache_tokens.resize(slot.cache_tokens.size() -
                                             n_discard);
        ...
        for (auto &slot : slots) {
            ...
            llama_batch_add(batch, slot.sampled,
                            system_tokens.size() + slot_npast, {slot.id}, true);
            slot.n_past += 1;
        }
        ...
        if (params.cont_batching || batch.n_tokens == 0) {
            for (auto &slot : slots) {
                ...
                // response note: infill mode allows empty prompt
                if (slot.state == IDLE && slot.command == LOAD_PROMPT &&
                    !has_prompt && !slot.infill) {
                    slot.release();
                    slot.print_timings();
                ...
                if (slot.state == IDLE && slot.command == LOAD_PROMPT) {
                    ...
                    if (slot.infill) {
                        ...
                        prompt_tokens =
                            tokenize(slot.prompt,
                                     system_prompt.empty() &&
                                         add_bos_token); // add BOS if there
                                                         // isn't system prompt
                    }
                    ...
                    if (slot.ga_n == 1 && slot.n_prompt_tokens >= slot.n_ctx) {
                        ...
                        const int erased_blocks =
                            (slot.n_prompt_tokens - slot.params.n_keep -
                             n_block_size) /
                            n_block_size;
                    ...
                    if (!slot.params.cache_prompt) {
                        ...
                        // the last token of the cache is not in the KV cache
                        // until the next call to llama_decode (it was sampled,
                        // pushed into the "cache_tokens", but not yet put in
                        // the context)
                        if (slot.n_past > 0 &&
                    ...
                    if (slot.n_past == slot.n_prompt_tokens &&
                        ...
                        LOG_INFO(
                            "we have to evaluate at least 1 token to generate "
                            "logits",
                            {{"slot_id", slot.id}, {"task_id", slot.task_id}});
                        slot.n_past--;
                        if (slot.ga_i > 0) {
                            slot.n_past_se--;
                        }
            ...
            for (auto &slot : slots) {
                if (slot.ga_n != 1) {
                    ...
                    while (slot.n_past_se >= slot.ga_i + slot.ga_w) {
                        ...
                        const int bd =
                            (slot.ga_w / slot.ga_n) * (slot.ga_n - 1);
                        const int dd =
                            (slot.ga_w / slot.ga_n) - ib * bd - slot.ga_w;
                        ...
                        llama_kv_cache_seq_add(ctx, slot.id, slot.ga_i,
                                               slot.n_past_se, ib * bd);
                        llama_kv_cache_seq_div(
                            ctx, slot.id, slot.ga_i + ib * bd,
                            slot.ga_i + ib * bd + slot.ga_w, slot.ga_n);
...
static void server_print_usage(const char *argv0, const gpt_params &params,
    ...
    printf("  --threads-http N          number of threads in the http server "
           "pool to process requests (default: max(hardware concurrency - 1, "
           "--parallel N + 2))\n");
    printf("  -c N, --ctx-size N        size of the prompt context (default: "
           "%d)\n",
           params.n_ctx);
    printf("  --rope-scaling {none,linear,yarn}\n");
    printf("                            RoPE frequency scaling method, "
           "defaults to linear unless specified by the model\n");
    ...
    printf("  --yarn-ext-factor N       YaRN: extrapolation mix factor "
           "(default: 1.0, 0.0 = full interpolation)\n");
    ...
    if (llama_supports_mmap()) {
        printf("  --no-mmap                 do not memory-map model (slower "
               "load but may reduce pageouts if not using mlock)\n");
    }
    printf("  --numa TYPE               attempt optimizations that help on "
           "some NUMA systems\n");
    printf("                              - distribute: spread execution "
           "evenly over all nodes\n");
    printf("                              - isolate: only spawn threads on "
           "CPUs on the node that execution started on\n");
    printf("                              - numactl: use the CPU map provided "
           "my numactl\n");
    if (llama_supports_gpu_offload()) {
        ...
        printf("                            how to split the model across "
               "multiple GPUs, one of:\n");
        printf("                              - none: use one GPU only\n");
        printf("                              - layer (default): split layers "
               "and KV across GPUs\n");
        printf("                              - row: split rows across GPUs\n");
        printf("  -ts SPLIT --tensor-split SPLIT\n");
        printf("                            fraction of the model to offload "
               "to each GPU, comma-separated list of proportions, e.g. 3,1\n");
    ...
    printf("  --lora-base FNAME         optional model to use as a base for "
           "the layers modified by the LoRA adapter\n");
    printf("  --host                    ip address to listen (default  "
           "(default: %s)\n",
    ...
    printf("  --embedding               enable embedding vector output "
           "(default: %s)\n",
    ...
    printf("                            set a file to load a system prompt "
           "(initial prompt of all slots), this is useful for chat "
           "applications.\n");
    printf("  -ctk TYPE, --cache-type-k TYPE\n");
    printf("                            KV cache data type for K (default: "
           "f16)\n");
    printf("  -ctv TYPE, --cache-type-v TYPE\n");
    printf("                            KV cache data type for V (default: "
           "f16)\n");
    printf("  --mmproj MMPROJ_FILE      path to a multimodal projector file "
           "for LLaVA.\n");
    printf("  --log-format              log output format: json or text "
           "(default: json)\n");
    printf("  --log-disable             disables logging to a file.\n");
    printf("  --slots-endpoint-disable  disables slots monitoring endpoint.\n");
    printf("  --metrics                 enable prometheus compatible metrics "
           "endpoint (default: %s).\n",
           sparams.metrics_endpoint ? "enabled" : "disabled");
    printf("\n");
    printf(
        "  -n, --n-predict           maximum tokens to predict (default: %d)\n",
        params.n_predict);
    ...
    printf("  -gan N, --grp-attn-n N    set the group attention factor to "
           "extend context size through self-extend(default: 1=disabled), used "
           "together with group attention width `--grp-attn-w`\n");
    printf("  -gaw N, --grp-attn-w N    set the group attention width to "
           "extend context size through self-extend(default: 512), used "
           "together with group attention factor `--grp-attn-n`\n");
    printf("  --chat-template JINJA_TEMPLATE\n");
    printf("                            set custom jinja chat template "
           "(default: template taken from model's metadata)\n");
    printf("                            Note: only commonly used templates are "
           "accepted, since we don't have jinja parser\n");
    printf("\n");
}
        ...
        if (arg == "--port") {
            ...
            if (!verify_custom_template(argv[i])) {
                fprintf(
                    ...
                    argv[i]);
                fprintf(stderr, "note: llama.cpp does not use jinja parser, we "
...
static json
format_partial_response(llama_server_context &llama, server_slot *slot,
...
int main(int argc, char **argv) {
    ...
    svr.Get(
        "/health", [&](const httplib::Request &req, httplib::Response &res) {
            ...
            case SERVER_STATE_LOADING_MODEL:
                res.set_content(R"({"status": "loading model"})",
                                "application/json");
                res.status = 503; // HTTP Service Unavailable
                break;
            case SERVER_STATE_ERROR:
                res.set_content(
                    R"({"status": "error", "error": "Model failed to load"})",
                    "application/json");
    ...
    if (sparams.slots_endpoint) {
        svr.Get("/slots",
                [&](const httplib::Request &, httplib::Response &res) {
                    ...
                    res.set_content(result.result_json["slots"].dump(),
                                    "application/json");
                    res.status = 200; // HTTP OK
                });
    }
    ...
    if (sparams.metrics_endpoint) {
        svr.Get("/metrics", [&](const httplib::Request &,
            ...
            uint64_t n_prompt_tokens_processed =
                data["n_prompt_tokens_processed"];
                ...
                {"gauge",
                 {{{"name", "prompt_tokens_seconds"},
                  ...
                  {{"name", "predicted_tokens_seconds"},
                   ...
                   {"value", n_tokens_predicted ? 1e3 / t_tokens_generation *
                                                      n_tokens_predicted
                                                : 0}},
    ...
    if (sparams.api_keys.size() == 1) {
        log_data["api_key"] =
            "api_key: ****" +
            sparams.api_keys[0].substr(sparams.api_keys[0].length() - 4);
    } else if (sparams.api_keys.size() > 1) {
        log_data["api_key"] =
            "api_key: " + std::to_string(sparams.api_keys.size()) +
            " keys loaded";
    }
    ...
    auto validate_api_key = [&sparams](const httplib::Request &req,
        ...
        if (auth_header.substr(0, prefix.size()) == prefix) {
            ...
            if (std::find(sparams.api_keys.begin(), sparams.api_keys.end(),
                          received_api_key) != sparams.api_keys.end()) {
                return true; // API key is valid
            }
        }

        // API key is invalid or not provided
        res.set_content("Unauthorized: Invalid API Key",
            ...
            [&llama](const httplib::Request &req, httplib::Response &res) {
                ...
                json data = {{"user_name", llama.name_user.c_str()},
                             ...
                             {"default_generation_settings",
                              llama.default_generation_settings_for_props},
                             {"total_slots", llama.params.n_parallel}};
                res.set_content(data.dump(), "application/json; charset=utf-8");
            });

    svr.Post("/completion", [&llama,
                             &validate_api_key](const httplib::Request &req,
                                                httplib::Response &res) {
        res.set_header("Access-Control-Allow-Origin",
                       req.get_header_value("Origin"));
        ...
        if (!json_value(data, "stream", false)) {
            ...
            if (!result.error && result.stop) {
                ...
                res.status = 404;
                res.set_content(result.result_json["content"],
                                "text/plain; charset=utf-8");
            }
    ...
    svr.Get("/v1/models", [&params, &model_meta](const httplib::Request &req,
                                                 httplib::Response &res) {
        res.set_header("Access-Control-Allow-Origin",
                       req.get_header_value("Origin"));
    ...
    const auto chat_completions = [&llama, &validate_api_key,
                                   &sparams](const httplib::Request &req,
                                             httplib::Response &res) {
        res.set_header("Access-Control-Allow-Origin",
                       req.get_header_value("Origin"));
        if (!validate_api_key(req, res)) {
        ...
        json data = oaicompat_completion_params_parse(
            llama.model, json::parse(req.body), sparams.chat_template);
        ...
        if (!json_value(data, "stream", false)) {
            ...
            if (!result.error && result.stop) {
                json oaicompat_result =
                    format_final_response_oaicompat(data, result);

                res.set_content(
                    oaicompat_result.dump(-1, ' ', false,
                                          json::error_handler_t::replace),
                    "application/json; charset=utf-8");
            } else {
                res.status = 500;
            ...
            const auto chunked_content_provider = [task_id, &llama](
                                                      ...
                                                      httplib::DataSink &sink) {
                while (true) {
                    task_result llama_result =
                        llama.queue_results.recv(task_id);
                    if (!llama_result.error) {
                        std::vector<json> result_array =
                            format_partial_response_oaicompat(llama_result);

                        for (auto it = result_array.begin();
                             it != result_array.end(); ++it) {
                            if (!it->empty()) {
                                const std::string str =
                                    "data: " +
                                    it->dump(-1, ' ', false,
                                ...
                                LOG_VERBOSE("data stream", {{"to_send", str}});
                                if (!sink.write(str.c_str(), str.size())) {
                                    llama.queue_results.remove_waiting_task_id(
                                        task_id);
                                    return false;
                                }
                            }
    ...
    svr.Post("/infill", [&llama, &validate_api_key](const httplib::Request &req,
                                                    httplib::Response &res) {
        res.set_header("Access-Control-Allow-Origin",
                       req.get_header_value("Origin"));
        if (!validate_api_key(req, res)) {
    ...
    svr.Post("/tokenize", [&llama](const httplib::Request &req,
                                   httplib::Response &res) {
        res.set_header("Access-Control-Allow-Origin",
                       req.get_header_value("Origin"));
    ...
    svr.Post("/embedding", [&llama](const httplib::Request &req,
        ...
        llama.request_completion(
            task_id,
            {{"prompt", prompt}, {"n_predict", 0}, {"image_data", image_data}},
            false, true, -1);
        ...
        // send the result
        return res.set_content(result.result_json.dump(),
                               "application/json; charset=utf-8");
    });

    svr.Post("/v1/embeddings", [&llama](const httplib::Request &req,
                                        httplib::Response &res) {
        res.set_header("Access-Control-Allow-Origin",
                       req.get_header_value("Origin"));
        const json body = json::parse(req.body);
        ...
        if (body.count("input") != 0) {
            ...
            if (prompt.is_array()) {
                ...
                for (const json &elem : prompt) {
                    ...
                    llama.queue_results.add_waiting_task_id(task_id);
                    llama.request_completion(
                        task_id, {{"prompt", elem}, {"n_predict", 0}}, false,
                        true, -1);
                    ...
                    json embedding = json{
                        {"embedding", json_value(result.result_json,
                ...
                return res.set_content(result.dump(),
                                       "application/json; charset=utf-8");
            }
        } else {
            prompt = "";
        ...
        llama.queue_results.add_waiting_task_id(task_id);
        llama.request_completion(
            task_id, {{"prompt", prompt}, {"n_predict", 0}}, false, true, -1);
        ...
        json data = json::array(
            {json{{"embedding",
                   json_value(result.result_json, "embedding", json::array())},
                  {"index", 0},
                  {"object", "embedding"}}});
    ...
    svr.new_task_queue = [&sparams] {
        return new httplib::ThreadPool(sparams.n_threads_http);
    };
    ...
    llama.queue_tasks.on_new_task(
        std::bind(&llama_server_context::process_single_task, &llama,
                  std::placeholders::_1));
    llama.queue_tasks.on_finish_multitask(
        std::bind(&llama_server_context::on_finish_multitask, &llama,
                  std::placeholders::_1));
    llama.queue_tasks.on_run_slots(
        std::bind(&llama_server_context::update_slots, &llama));
    llama.queue_results.on_multitask_update(std::bind(
        &llama_server_queue::update_multitask, &llama.queue_tasks,
        std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    shutdown_handler = [&](int) { llama.queue_tasks.terminate(); };
    ...
    auto console_ctrl_handler = +[](DWORD ctrl_type) -> BOOL {
        return (ctrl_type == CTRL_C_EVENT) ? (signal_handler(SIGINT), true)
                                           : false;
    };
